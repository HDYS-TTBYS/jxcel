...
pub fn run() -> Result<(), StartupError> {
    ...
    // 手順 4.3 の続き: マクロの実行の導線の登録（`macro-runtime` スペックの要件 2.1。タスク
    //   4.3）。**7.4 と同じ登録口**へ `マクロ > 実行…` を 1 件足す。選択されたときの処理は、
    //   7.5 が解決した活性化の対象ウィンドウへ `MACRO_RUN_REQUESTED_EVENT` を送るだけであり、
    //   一覧の提示・マクロの選択・結果の提示はフロントエンドの面（4.4）が 4 つの `macro_*`
    //   コマンドを通して行う。**アクセラレータを付けない**（打鍵の導線を要求されていない）。
    //   実行の項目は**実行できるマクロが 1 つも無くても無効化しない** — 述語の評価は
    //   フォーカスが移るたびに走り、その時点で文書を読むとイベントループが実行中のマクロの
    //   ロックを待つ（要件 2.2）。導線を出すかどうかは、一覧を持っている面が決める（要件 2.7）。
    commands::macro_install(app.handle());
