...
pub fn render_heartbeat(
    ...
    // 残す）。
    if withdraw {
        // **`画面=` をこの行にも載せる。** 期限を超えて描画が成立した場合、10.4 の 3 OS の
        // 描画確認（`scripts/check-x11-render.sh` と CI の macOS / Windows の段)は
        // **どの画面が描画されたか**をこの行から読む。以前はここに画面が無かったため、
        // 期限をわずかに超えただけの起動（遅いランナーで起きる）は「描画された画面を証明
        // できない」として落ちていた（実測: 2026-09-12 の macOS のランナーで `期限=3000 ms
        // 経過=3142 ms`）。**判定（`NoPaint`）と印（`render.fallback`）は変えない** — 期限の
        // 時点で描画が成立していなかった事実は動かない（下の doc のとおり）。
        log::warn!(
            target: LOG_TARGET,
            "期限超過のあとに描画の通知が届いた（不成立の提示を取り下げる）: label={} 画面={}",
            label.as_str(),
            screen_text(request.screen.as_deref()),
        );
...
///
/// **成立・不成立・期限超過後の通知のどの行も `画面=<識別子>` を必ず出す。**これが
/// 「初回描画が成立したこと」と「**どの画面が**描画されたか」を同じ 1 つの記録で判定できる
/// ようにする（tasks.md 10.4 の 3 OS の描画確認）。値は通知が報告した**実際に描画されていた
/// 画面**であり、起動時に要求した識別子ではない（要求が未登録ならシェルは既定の画面へ落ちる
/// ので、両者は一致しない。9.7）。報告が無い通知では `(報告なし)` と出す — **空にしない**
/// ので、10.4 の段は「画面が分からないままの成功」を成立として扱えない。
///
/// **期限超過後に届いた通知の行にも `画面=` を載せる**（[`render_heartbeat`]）。遅いランナー
/// では期限（3 秒）をわずかに超えて描画が成立することがあり、その場合も 10.4 の段は
/// **どの画面が描画されたか**を読める必要がある（判定は `NoPaint` のままであり、
/// 「期限の時点で描画が成立していなかった」事実は変わらない）。
