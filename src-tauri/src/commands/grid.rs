...
/// **`document_format::Document` を名指す関数は置かない** — コマンド層の境界の型は
/// `app-shell` の IPC が定める（`ipc-contract.md`）ため、文書の型をコマンドの署名へ出さない。
/// 表を組むのは文書を読む 2 箇所（開く経路と表示の指定を変える経路）の内側であり、そこで型は推論される。
/// （2026-09-18 訂正: 以前は「`document-format` は dev-dependency だから参照できない」を理由に
/// 挙げていたが、マクロのホストの縫い目が `document_format::SheetId` を名指すため通常依存へ移した
/// （`src-tauri/src/macro_host.rs`）。理由を依存の種別から境界の規約へ直した。規則自体は不変。）
...
// コンパイル時に固定する。
//
// **保持のロックの規律の検査（「実行の間は握らない」= 要件 2.2）は本モジュールには無い。**
// あれは実行の本体（`commands/macro.rs` の `answer_run`）を駆動する検査であり、実行 1 回に
// つき `macro_run:` の記録を 1 行出すため、記録を数える検査と直列化する鍵
// （`macro.rs` の `mod tests` の `RUN_RECORD_LOCK`）を取れる側に置いてある —
// `macro.rs` の `実行中でも表の窓は止まらない`（なぜそこに在るかの理由もそちらの doc にある）。
// ---------------------------------------------------------------------------
