...
- **端末側（`document-format` / `schema-engine`）の未解決の申し送りは各スペックの `tasks.md` に残る**（次にそのクレートを触るタスクが拾う性質のもの。**明示的に「未対応の申し送り」と書かれているのは `schema-engine` の `unique::scan` / `refs::scan` が未整列の列を渡されると黙って取りこぼす点**）

**CI・実機でしか閉じられない**

- macOS / Windows の実行時（`app-shell` / `document-session` / `data-grid` / `macro-runtime` の段と実測値）と、配布物（AppImage / deb）の大きさの増分（`macro-runtime`）
- **`cargo test --features verification-triggers` がどの段にも無い**（引き金の経路そのものは 3 OS の実起動観測の段で走っている。現状は `verification.md` に記録）
...
- [x] macro-runtime -- deno_core の埋め込み、TS トランスパイル経路、ホスト API、実行の隔離。Dependencies: document-format, schema-engine, app-shell
...
**2026-09-18 の更新（`macro-runtime` の実装完了）**: `macro-runtime` の 21 サブタスクが完了した（フィーチャー横断の検証は GO）。**依存が満たされているスペックは 4 本**になった（`schema-editor` / `version-control` / `export-templates` / `form-builder`）。**`macro-runtime` を待っていた 4 本（`custom-types` / `macro-stdlib` / `macro-editor-lsp` / `formula-engine`）がここから開く** — Wave 4 はこのスペックが律速だった。**MVP の範囲（Wave 1 + Wave 2 + document-session + data-grid + schema-editor）は変わらず、残るのは `schema-editor` の 1 本**である。
