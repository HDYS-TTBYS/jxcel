...
## Current State
document-format は実装完了（2026-09-12）。ドキュメント構造（File / Sheet / Schema / Row）は揃っているが、
**スキーマの意味論は意図的に空けたまま**であり、本スペックが次の 4 つの継ぎ目を引き継ぐ。

- **スキーマ・ペイロードは不透明**: エントリ `schemas/<sheet-ulid>.json` のエンベロープ形
  `{ "root": <opaque>, "types": [ { "id": "<ULID-26>", "definition": <opaque> } ] }` は
  document-format が所有するが、`root` と `definition` の中身は一切解釈されない
  （`model::SchemaPart`）。抽出されるのは型定義の識別子と `{"$ref": "<ターゲット>"}` という
  参照マーカーの出現だけである。**型の意味論と `$ref` の解決規則は本スペックが所有する**
- **セル値は閉じた 8 変種**（`value::CellValue`: Null / Bool / Int / Float / Decimal / Text /
  Nested / Attachment）。「どの列がどの変種を使うか」「`Decimal` の桁数と有効性」は
  document-format 側が明示的に本スペックへ委ねている
- **「移行」の語が二重になっている**: document-format の `migration` モジュールは
  **形式バージョン**（`major.minor`）の移行であり、本スペックの「スキーマ変更時のデータ移行」とは
  別物。design では名前を分けること
- **決定的 JSON 出力と未知フィールドの保持は document-format が所有済み**。スキーマ宣言の表現は
  その上に乗せるだけでよく、決定性を本スペックで作り直す必要はない
