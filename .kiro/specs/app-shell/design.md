...
- **アプリ全体の終了の経路（`request_exit`）** — いまは**未保存が問われずに終了する**（ウィンドウを閉じる操作とは**別経路**）。`document-session` はこれを自スペックの境界外とし、`design.md` の Risks と `requirements.md` の Out of scope に記録している（**利用者のデータが失われる経路である**）。終了を拒否可能にする設計変更は本スペック側の仕事であり、**変更するときは `document-session` の未保存の問い（終了前の問いの差し替え口・`CloseDenied` の再試行）と同じ扱いを設計すること**
