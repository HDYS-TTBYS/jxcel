...
- **10.6（Windows: アクセラレータは WebView2 に消費される・重要／2026-09-12 の CI で確定）**: Windows では**ネイティブのメニューアクセラレータ（`Ctrl+Shift+J` 等）が素の利用者操作では作用しない**。理由は基盤の作り: アクセラレータを解釈するのはホストウィンドウのメッセージループであり（tauri 2.11.5 の `msg_hook` が `TranslateAcceleratorW(msg.hwnd, haccel, msg)` を呼び、`GetKeyState` で修飾キーを見て一致すれば `msg.hwnd` へ `WM_COMMAND` を送る — Wine の `win32u/menu.c` の `translate_accelerator` で確認）、**WebView2 は自分の子ウィンドウにキーボードフォーカスを持ってアクセラレータキーを自分の経路で消費する**（上流の既知の未解決の制限: tauri-apps/wry#451、tauri#6981）。実測: `keybd_event` によるグローバルな鍵注入では**記録行が 1 行も現れなかった**（2026-09-12 の Windows のランナー）。**したがって要件 3.5 の「ショートカットの操作を対象ウィンドウにのみ適用する」は Windows では実利用で発火しない** — アプリ側で直すなら、**フォーカスが実際にある webview の中でキーを扱い、メニューと同じ処理へ渡す**経路が要る（新しいコマンド＝通信境界・capability・生成物に触れるため、本スペックの作業範囲を越える。**未決の積み残しとして記録する**）。CI の 10.6 の段は、**修飾キーを実際に押したまま対象ウィンドウへ `WM_KEYDOWN`/`WM_KEYUP` を送る**（`JxcelMenuApi.SendShortcutTo`）ことで、**アクセラレータが基盤へ登録され、活性化の経路（メッセージループ → `TranslateAcceleratorW` → `WM_COMMAND` → muda → 登録元）が働くこと**と、**対象がフォーカスに追随すること**を測る（Linux の段が `XSendEvent` で合成イベントを送るのと同じ判断）。**この段は「素の利用者が Windows でキーを押せば作用する」ことを主張しない。**
...
- **10.5（macOS: ウィンドウ集合の落ち着き待ち／2026-09-12 の CI で確定）**: macOS の段の「引数なしの再実行でウィンドウが増えない」の判定は、**数ではなくウィンドウ識別子（`kCGWindowNumber`）の集合**を見て、**観測窓（5 秒）の最後の 2 標本が一致し、かつ起動前の集合と一致すること**を要求する。1 標本だけの比較にしていたため、ウィンドウサーバの一時的な作り直しで偽の失敗になった（実測: 2026-09-12 の macOS のランナー。`引数なしの 2 つ目の起動でウィンドウ数が 2 から変わった`）。**集合で見るので入れ替わりも数え落とさず、持続する差は引き続き失敗する**（Linux の `stable_wait` と同じ判断）。
