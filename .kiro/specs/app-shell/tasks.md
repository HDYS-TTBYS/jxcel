...
- **10.6（Linux: 1 本で 3 主張・重要）**: `scripts/check-menu-shortcut.sh` が 1 回の実行で (1) **AT-SPI で配布物（AppImage）の実メニュー木を読む**（`診断` の 3 項目＋`ファイル` の `終了`/`開く…`。**キーバインドは `GetKeyBinding` で読む**（`GetActions` は呼ばない — 下の「`GetActions` は呼ばない」を参照）。同時に**配布物に検証専用項目が無いこと**も AT-SPI 側から確認＝5.4 の片付けの独立確認）、(2) **`org.a11y.atspi.Action.DoAction` で活性化**して通知の 2 行（`メニュー項目が選択された: 登録元=… 対象ウィンドウ=…` と登録元の後続要求）を要求、(3) **2 枚のウィンドウでフォーカス追随**（`empty-1` / `doc-1` に `XSetInputFocus` + `XSendEvent`。**1 回の押下で作用はちょうど 1 枚、他方への作用は 0 件**、フォーカスを戻すと追随）を行う。**AT-SPI の読みは「アプリのモデル」ではなく「基盤が描いたメニュー」**である（配布物に検証用の自己申告文字列が 0 件であることを strings で確認）。活性化の記録行は**唯一の seam `MenuRegistry::dispatch`** から出る。
- **10.6（キーバインドは `GetKeyBinding` で読む・重要／2026-09-12 の CI で確定）**: (1) は AT-SPI から**キーバインドの綴り**（`<Primary>q` 等）も読む。ただし **`org.a11y.atspi.Action.GetActions` を呼んではならない** — 固定している最も古い対象環境（ubuntu-22.04）の `libatk-bridge-2.0.so.0` は **2.38.0** で、その `impl_GetActions` は `a(sss)` へ 4 つ目の文字列を書こうとし、libdbus が `Array or variant type requires that type end_struct be written, but string was written.` で **abort する**（＝呼び出し側からは「相手が返事の前に D-Bus から消えた」と見える。上流の修正は GNOME/at-spi2-core `dc0dc331`（2022-04-07、2.38 より後）で 22.04 には入らない。**アプリのバグでも同梱ライブラリの問題でもない** — 配布物のものでもホストのものでも 2.38 なので、実行ファイルを展開してホストのライブラリを読ませる処置（967b78d で入れた）では直らなかった）。**要るのはキーバインドの文字列 1 本だけ**なので、応答が `s` である **`GetKeyBinding(index)`** で読む（構造体の組み立てを通らない）。AT-SPI の仕様（`xml/Action.xml`）が「`GetActions` は各項目について `GetLocalizedName` / `GetDescription` / `GetKeyBinding` を呼ぶのと等価」と定めているので**読める内容は同じ**であり、この段の主張は変わらない。アクションを持たない項目は `Action.NActions`（読取専用プロパティ）で見て空を返す（`verification.document-only` の対照）。**実測**: ホスト（26.04）で `atspi-tree` が `開く…=<Primary>o` / `終了=<Primary>q` / `検証: 対象ウィンドウを記録=<Primary><Shift>j` / `検証: ドキュメント付きのみ=（キーバインドなし）` を返し、`atspi-verify-verification` が 7 項目すべて成立した（22.04 の atk-adaptor 2.38 が `GetKeyBinding` を実装しているかは CI が判定する）。**以前は (3) の「表示の表記」が AT-SPI のキーバインドを読むと書いてあり、967b78d で読み取りを消したときにこの記述が実際と食い違っていた**（この復元で再び一致する）。
...
- **10.6（Windows: アクセラレータは WebView2 に消費される・重要／2026-09-12 の CI で確定）**: Windows では**ネイティブのメニューアクセラレータ（`Ctrl+Shift+J` 等）が webview にフォーカスがある状態では作用しない**。理由は基盤の作り: アクセラレータを解釈するのはホストウィンドウのメッセージループであり（tauri 2.11.5 の `msg_hook` が `TranslateAcceleratorW(msg.hwnd, haccel, msg)` を呼び、`GetKeyState` で修飾キーを見て一致すれば `msg.hwnd` へ `WM_COMMAND` を `wParam = 0x10000 | cmd` で送る — Wine の `win32u/menu.c` の `translate_accelerator` で確認）、**WebView2 は自分の子ウィンドウにキーボードフォーカスを持ってアクセラレータキーを自分の経路で消費する**（上流の既知の未解決の制限: tauri-apps/wry#451、tauri#6981）。実測（2026-09-12 の Windows のランナー）: `keybd_event` によるグローバルな鍵注入でも、修飾キーを実際に押したまま対象ウィンドウへ `WM_KEYDOWN` を送る形でも、**記録行が 1 行も現れなかった**。**したがって要件 3.5 の「ショートカットの操作を対象ウィンドウにのみ適用する」は、Windows では素の利用者が webview にフォーカスを置いたままキーを押しても発火しない** — アプリ側で直すなら、**フォーカスが実際にある webview の中でキーを扱い、メニューと同じ処理へ渡す**経路が要る（新しいコマンド＝通信境界・capability・生成物に触れるため、本スペックの作業範囲を越える。**未決の積み残しとして記録する**）。CI の 10.6 の Windows の段は、**アクセラレータが生むのと同じメッセージ**（`WM_COMMAND` を `0x10000 | cmd` で）を前面化したウィンドウへ送ることで、**活性化が muda のサブクラス → 登録元 → 対象ウィンドウの解決へ到達し、対象がフォーカスに追随すること**（要件 3.5）と、**各ウィンドウのメニューにアクセラレータ付きの項目が乗っていること**（Win32 の項目文字列に `Ctrl+Shift+J` が埋まっている）を測る。**アクセラレータ表の照合そのものは主張しない** （実キーでの実測は Linux の段が `XSendEvent` で担う — `XTEST` はそのツールキットに届かない）。
- **10.6（表記の主張の限界・記録）**: 表示の表記は **AT-SPI が報告するキーバインド（`GetKeyBinding`）＋検証用の形の記録（基盤へ渡した解決済みの綴り `ctrl+KeyO` 等）＋キー送出による作用の実測**で確認する。**ポップアップの画素そのものは見ていない**（7.5 がローカルで実測済み）。macOS は AX に権限が要るため**アプリが `set_menu` へ渡した配置と綴りの記録＋クレートのテスト**で検証する（**AppKit の描画そのものは証明しない**と明記）。Windows は Win32 のメニュー列挙（項目の文字列にアクセラレータが埋まっている）＋`WM_COMMAND`。
...
- **10.6（堅牢化）**: mutter が最大化時に toplevel を作り直す事象への対策として**ウィンドウ集合が 2 回続けて同じになるまで待つ**、開始前に同題名のウィンドウが 0 件であることを要求、X エラーを説明可能な失敗に変換する（キーバインドの `<Primary>` / `<Control>` の許容は**必要である** — ランナーの GTK 版によって綴りが変わる。AT-SPI の読みを復元したのでこの許容も戻した）。


- **10.5（macOS: 見えているウィンドウの集合で見る／2026-09-12 の CI で確定）**: macOS の段の「引数なしの再実行でウィンドウが増えない」と「閉じた後はウィンドウが 0 枚」の判定は、**`kCGWindowIsOnscreen` が真・`kCGWindowAlpha` が正・通常レイヤー（0）・100x100 以上**のウィンドウの**識別子の集合**で行う。`.optionAll` の一覧にはプロセスが持つ**見えないウィンドウ**も含まれ、レイヤーと大きさだけのフィルタではそれを数えてしまう（実測: ウィンドウを 1 枚も開いていない状態でも 1 枚残り、`0 枚になるのを待つ` 検査と`引数なしの再実行` の検査が偽の失敗になった。1.5 / 10.4 は「1 枚以上」しか見ないので影響していない）。あわせて**観測窓（5 秒）の最後の 2 標本が起動前の集合と一致すること**を要求し（Linux の `stable_wait` と同じ判断）、一時的な作り直しを許しつつ**持続する差は失敗させる**。集合なので入れ替わりも数え落とさない。
- **10.6（macOS: 期待する位置は macOS の慣習・2026-09-12 の CI で確定）**: macOS の 10.6 の段は配置の記録に **`app-shell.quit(jxcel > 終了, ショートカット=super+KeyQ)`・`verification.*(jxcel > 検証: …, …)`・`app-shell.open-document(ファイル > 開く…, …)`・診断の 3 項目（`診断 > …`）** を要求する。組み込みの「終了」と検証専用の 2 項目は **`builtin_quit_path()` が macOS ではアプリケーションメニュー（`jxcel`）を指す**ためそこへ置かれる（`menu.rs` の `builtin_quit_path` とクレートのテスト `the_builtin_quit_item_is_placed_in_the_platform_conventional_submenu` が固定している）。**以前この一覧は Linux / Windows と同じ「ファイル > 終了」を要求していた** — macOS のアプリ全体のメニューでは成立せず、この段が初めて最後まで走った 2026-09-12 に露見した（それ以前は手前の段で落ちていた）。
...
- **10.1（YAML の検証手段・2026-09-12 に更新）**: **actionlint** で `ci.yml` / `bench.yml` を検査する（exit 0）。**qlty は actionlint 1.7.9 と shellcheck 0.11.0 を導入済みである**（`qlty plugins list` が ✔ を返し、実体は `~/.qlty/cache/tools/actionlint/1.7.9-dfdef16ea705/actionlint` と `~/.qlty/cache/tools/shellcheck/0.11.0-f6932738ad3a/shellcheck` にある）— **以前この行に書いていた「qlty に actionlint の導入が無い」「このホストに shellcheck は無い」は誤り**であり、`scripts/ci/` の抽出時に qlty の shellcheck が実際に 21 件の指摘（SC2148: shebang 不在）を出した。**ワークフローを触ったら actionlint を、`*.sh` を足したら `qlty check` を当てる。**
- **10.1（`ci.yml` の分割・2026-09-12）**: 各行が数百行になる検証の段（ブロックの中に長い bash / PowerShell と Swift の heredoc を埋めていた）を **`scripts/ci/` のスクリプトへ逐語で移し、ワークフローの段はそれを 1 行で呼ぶだけ**にした。**3758 行 → 1078 行**（`run:` の行数は 2588 → 約 30）。配置は `scripts/ci/linux|macos|windows/`（OS ごと）と `common/`（3 OS 共通）で、macOS の 3 段の Swift のスパイクは同じディレクトリの `*.swift`（段の中の heredoc をやめた）。**段の並び・名前・`if:`・`shell:`・`env:`・`with:` は不変**であり、`run:` の本文は 27 段すべてで**逐語一致**を機械照合した（移したコメントも各行の存在を照合）。**振る舞いを変えないための処置**: Actions が台本へ与えていた `-e`（`shell: bash` なら `-o pipefail`）と `$ErrorActionPreference = 'Stop'` は、自分で `set -…` を書いていない段（`install-system-deps.sh` と `verify-bundle-window.sh` / 各 `.ps1`）の冒頭が持つ。**検証**: actionlint exit 0、qlty（shellcheck 含む）No issues、`bash -n` 全 21 本、**PowerShell 6 本は `mcr.microsoft.com/powershell:7.4` の `[Parser]::ParseFile` で構文解析して 0 エラー**（ホストに pwsh が無いため）。**ジョブもワークフローも増やしていない**（要件 6.2）。
...
- **9.6（提示）**: 画面は `src/features/empty/EmptyWindowScreen.tsx`（シェルの登録簿に 1 件追加）。ドキュメントの無いウィンドウでは `data-shell-screen=empty-window` で見出し「ドキュメントが関連付けられていません」＋**「新規作成」「既存ファイルを開く…」**の 2 操作を出す。ドキュメントのあるウィンドウでは関連付けの注記だけで操作は出さない（**ドキュメント画面は下流スペックの持ち場であり、偽の UI を作らない**）。**（2026-09-14 の更新: 見出しは「ドキュメントを保持していません」へ変わった — 下の「`document-session` 完了に伴う更新」を参照。2 操作を出す受入自体は不変。）**
- **9.6（新規作成は正直に）**: ドキュメントを所有する機能がまだ無いので、押しても**「このアプリケーションには、ドキュメントを所有する機能がまだ組み込まれていません」という段落を出すだけ**（コマンドを呼ばず、ログも出さず、ウィンドウも作らない＝**偽の成功をしない**。黙って何もしないのでもない）。**（2026-09-14 の更新: `document-session` が `document_new` を実装したため、この段落は撤去し、押すと本物の新規作成コマンドを呼ぶようになった — 下の「`document-session` 完了に伴う更新」を参照。「偽の成功をしない」という 9.6 の精神は `Created` / `Refused{reason}` / 封筒の失敗を別々に提示する形で保たれている。）**
- **9.6（開く経路）**: 7.7 の `pick_document_file` を**生成された名前**で呼ぶ。実測: 選択器は**操作したウィンドウに親子付け**（`WM_TRANSIENT_FOR` = そのウィンドウ、`_NET_WM_STATE_MODAL`、`_NET_WM_WINDOW_TYPE_DIALOG`）、選んだ位置は委譲先へ届く（検証用宿主が `[検証] 引き渡しを受けた: ウィンドウ = … 位置 = …`）、取り消しは「利用者が選択を取り消した」。**画面の文言は「引き渡し済み／取り消し／拒否／通信失敗」で区別できる**（区別は**結果行の本文**にあり、属性ではない — `data-testid` は `jxcel-empty-open-result`（引き渡し済み / 取り消し / 拒否）と `jxcel-empty-open-error`（封筒の失敗）の 2 つである。**`data-document-state` 属性は `document-session` の 4.3 以降セッションの状態（`Absent` / `Open` / `Unavailable` + `loading` / `failed`）を運ぶ**ので、この 4 区分はこの属性では読まない — 下の「`document-session` 完了に伴う更新」を参照）。**本機能はパスを読まない**（7.7 の契約を維持）。
...
- **9.6（`document-session` 完了に伴う更新・2026-09-14）**: この節の記述のうち、**画面の判定に関する部分は `document-session` の実装完了で事実に合わなくなった**（`document-session` の `design.md` の Revalidation Trigger「`window_document_state` の画面からの切り離し」）。実装（`src/features/empty/EmptyWindowScreen.tsx`）は判定の材料を `window_document_state`（生成要求の記録）から **`document_state`（セッションの状態）** へ置き換えた。したがって: 見出しは「ドキュメントが関連付けられていません」から**「ドキュメントを保持していません」**へ、`data-document-state` の語彙は `unassociated` / `associated` から **`"Absent"` / `"Open"` / `"Unavailable"`**（および**問い合わせ自体の状態**を表す `loading` / `failed`）へ変わった。**`window_document_state` は意図的に残す**（本節の「関連付けの判定」が定めたコマンドであり、削除も廃止もしない — 生成要求の記録という別の問いに答えるコマンドである）。**本節の 9.6 の受入（生成要求に関連付けの無いウィンドウに 2 操作を出す導線）自体は変わらない**。**この 2 つの画面の記述の更新は `document-session` の 4.3 が行った**（同スペックの 6.1 は本節の記述を事実に合わせて確認しただけであり、`src/shell/**` と `src/features/**` には触れていない）。
...
- **clippy の残件の片付け（2026-09-18・実測）**: `~/.rustup/toolchains/stable-x86_64-unknown-linux-gnu/bin/cargo clippy --workspace --all-targets --message-format=short`（ホストの stable 1.98.1）で実測し、**挙動を変えずに直せるものを直した**。生の警告行は `crates/app-shell` **27 件 → 5 件**、`crates/document-format` **134 件 → 1 件**（`benches/../tests/common/mod.rs` の重複出力を含む）。ファイル:行で一意にした箇所数では **21 → 5** と **30 → 1**。解消した lint は app-shell が `cloned_ref_to_slice_refs` 4 / `needless_range_loop` 3 / `needless_return` 2 / `byte_char_slices` / `type_complexity` / `single_match` / `while_let_loop` / `err_expect` / `manual_repeat_n` / `io_other_error` / `derivable_impls` 各 1、document-format が `doc_nested_refdefs` 6 / `useless_vec` 4 / `useless_format` 4 / `format_in_format_args` 4 / `type_complexity` 3 / `unnecessary_sort_by` 2 / `ptr_arg` / `manual_is_multiple_of` / `manual_find` / `excessive_precision` / `err_expect` / `chunks_exact_to_as_chunks` 各 1。**`unnecessary_sort_by` は安定整列のまま**（`sort_by` → `sort_by_key`、`sort_unstable_by` → `sort_unstable_by_key`。等しいキーの順序を変えない）。検証は `cargo test -p app-shell` と `cargo test -p document-format`（全段緑）。
- **10.1 の clippy 許容基準（2026-09-18 に確定）**: **`-D warnings` はワークスペース全体には掛けない**（`src-tauri` / `macro-runtime` / `document-session` / `schema-engine` / `data-grid` / `sidecar-smoke` に既存の警告が残るため）。掛けるのは **`crates/app-shell` と `crates/document-format` の 2 クレートだけ**で、level は `deny`（`-D warnings`）、**残す 3 lint・6 箇所だけを名指しで許容する**:
  `~/.rustup/toolchains/stable-x86_64-unknown-linux-gnu/bin/cargo clippy -p app-shell -p document-format --all-targets -- -D warnings -A clippy::assertions_on_constants -A clippy::result_large_err -A clippy::wrong_self_convention`
  - **`clippy::assertions_on_constants` 4 箇所**（`crates/app-shell/src/diagnostics.rs`: `retention_cap_is_50_megabytes` の `MAX_TOTAL_LOG_BYTES <= 50_000_000`、`retention_values_are_the_documented_ones` の `MAX_RETAINED_LOG_BYTES <= MAX_TOTAL_LOG_BYTES`、`retention_values_dominate_the_plugin_defaults_by_two_orders_of_magnitude` の 2 件）。**定数条件の `assert!` はこのテストの主張そのもの**（消すと「その値である」という主張が消える）なので残す。
  - **`clippy::result_large_err` 1 箇所**（`crates/app-shell/src/accelerator.rs` の `AcceleratorRegistry::insert`。`Err` 変種は 168 バイト）。直すには `Result<(), Box<AcceleratorConflict>>` への**公開署名の変更**が要り、`src-tauri/src/menu.rs` の呼び出し・doctest・下流へ波及する（`Box` 化自体は挙動を変えない）。
  - **`clippy::wrong_self_convention` 1 箇所**（`crates/document-format/src/lib.rs` のトレイト `DocumentFormatApi::from_parts(&self, …)`）。直すには**公開メソッドの改名**が要り、`crates/document-session`・`src-tauri` の呼び出しと design.md へ波及する。
  - **この 3 件を片付けたら `-A` を外し、2 クレートに `-D warnings` だけを掛ける**（そのとき `--workspace` へ広げるかは他クレートの残件次第）。
  - **`src-tauri` は 7.5 の「警告 0」が現在は成立していない**（2026-09-18 時点で `dead_code` 1・`format_in_format_args` 1・`type_complexity` 2・`bool_comparison` 1 の 5 箇所）。この片付けの対象範囲外なので baseline には含めず、そのまま残す。
- **doc の指摘の片付け（2026-09-18）**: ①**5.6 の申し送りどおり** `crates/app-shell/src/sidecar/supervisor.rs` の `with_grace` と `grace` の doc を「本番は `SIDECAR_SHUTDOWN_GRACE` を与える（Unix 3 秒 / Windows 300 ms）」へ直した（`DEFAULT_GRACE` は既定値であると併記。挙動は不変）②`crates/document-format/tests/common/mod.rs` の `DocumentView` の項目一覧が `* [`Self::x`]: …` と書かれており、Markdown がこれを**リンク参照定義**として食っていた（clippy `doc_nested_refdefs` 6 件、描画でも項目が消える）。`] — ` へ直して一覧として見えるようにした。**残る 2 件はこの範囲外（`src-tauri`）**: 5.4 の `src-tauri/src/lifecycle.rs` の `request_exit` が実行経路を `ControlFlow::ExitWithCode(0)` と書いている件と、6.2 の `src-tauri/src/ports.rs` の「setup フックは `Builder::build` の中で走る」の件。どちらも次に `src-tauri` を触るときに直すこと（tasks.md の 5.4 / 6.2 の記述どおり）。
...
- **3.3（`shutdown_all` のタイミング検証の上限・2026-09-12 の CI で確定）**: `crates/app-shell/tests/sidecar_lifecycle.rs` の `shutdown_all_returns_promptly_when_the_child_is_already_dead` の上限を **1 秒固定から `grace / 2`（2.5 秒）** にした。実測: Windows のランナーで 1 秒を超えて落ちた（「既に終了した子に猶予 5s を消費した」。**同じコードで 5 回は成功**）。経路は `terminate` のループで、`Child::try_wait` が終了を報告した後も **Job Object の `ActiveProcesses` が 0 になるまで待つ**（`job_windows.rs` の `exists`）。プロセスの終了から job の会計が 0 になるまでの遅れは、負荷の高いランナーで 1 秒を超えうる。**契約（猶予を使い切らない）は破られていない**ため、上限は猶予に対する比で表す（猶予を無条件に消費する退行は 5 秒かかるのでこの上限でも落ちる）。あわせて**実測値をメッセージに載せた**（次に落ちたときの証拠になる）。
