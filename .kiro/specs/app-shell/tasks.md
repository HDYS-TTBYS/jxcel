...
- **10.8（駆動と証拠・重要）**: 検証専用の駆動側は `src/shell/verificationBulk.ts`（`main.tsx` で結線。**`verification-triggers` の下だけ**）で、行数の一覧（既定 `100,100000`）ごとに**ちょうど 1 回** `invokeRaw("bulk_echo", payload)` を呼ぶ。アプリは呼び出しごとに `bulk_echo: 呼び出し元ウィンドウ = … 受信バイト数 = N` を、行数ごとに `一括転送の結果: {"byteIdentical":…, "invocations":…, "receivedBytes":…, "rows":…, "sentBytes":…}` を記録する。実測: **100 行 = 4,700 B / 100,000 行 = 4,700,000 B、いずれも invocations = 1**（2 つの大きさで合計 2 回＝**呼び出し回数は行数に比例しない**）、往復はバイト一致、全体 647 ms、初回描画 167〜173 ms。**64 MiB の上限に対して十分小さい**。Rust 側（`window/mod.rs` / `lifecycle.rs`）は検証専用の引き金と行数指定を運び、**埋め込みは素の行数一覧だけ**（注入対策のテスト付き）。
...
- **10.4（方式 A と対立の分析・重要）**: 要件 10.4 の完了状態は「**配布物**が両画面を描く」を求めるが、9.7 の到達経路は非既定の `verification-triggers` の下にしか無く、配布物に検証専用コードを入れない決定（5.4/8.2）と両立しない。**採用: A** — 配布物の検査は 1.5 の 3 段（ウィンドウ）＋ **8.2 の初回描画の記録行**（4.4 の OS 別ログ位置）のままにし、**両画面は検証用の形**（`npx tauri build --no-bundle --features verification-triggers` → `target/release/jxcel[.exe]` を `JXCEL_VERIFICATION_INITIAL_SCREEN=<id>` で起動）で確認する。**B（配布物に画面への導線を足す）は不採用**: 非実用画面に利用者向け項目を作ることになり 9.7 と `SmokeScreens` の契約に反し、macOS/Windows の UI 自動化（アクセシビリティ権限）に依存して CI が不安定になる。**トレードオフ（ci.yml の 10.4 節に明記）**: CI が実測するのは「配布物が起動して初回描画が成立する」ことと「**同じ埋め込み資産（同一チャンクハッシュ）を持つ検証用の形が両画面を描く**」ことである（配布物自身が両画面を描くことではない）。
