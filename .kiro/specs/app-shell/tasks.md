...
- **10.6（Linux: 1 本で 3 主張・重要）**: `scripts/check-menu-shortcut.sh` が 1 回の実行で (1) **AT-SPI で配布物（AppImage）の実メニュー木を読む**（`診断` の 3 項目＋`ファイル` の `終了`/`開く…`。`<Primary><Shift>v/l/e`・`<Primary>q/o` という**基盤が報告するキーバインド**付き。同時に**配布物に検証専用項目が無いこと**も AT-SPI 側から確認＝5.4 の片付けの独立確認）、(2) **`org.a11y.atspi.Action.DoAction` で活性化**して通知の 2 行（`メニュー項目が選択された: 登録元=… 対象ウィンドウ=…` と登録元の後続要求）を要求、(3) **2 枚のウィンドウでフォーカス追随**（`empty-1` / `doc-1` に `XSetInputFocus` + `XSendEvent`。**1 回の押下で作用はちょうど 1 枚、他方への作用は 0 件**、フォーカスを戻すと追随）を行う。**AT-SPI の読みは「アプリのモデル」ではなく「基盤が描いたメニュー」**である（配布物に検証用の自己申告文字列が 0 件であることを strings で確認）。活性化の記録行は**唯一の seam `MenuRegistry::dispatch`** から出る。
- **10.6（表記の主張の限界・記録）**: 表示の表記は **GTK が報告するキーバインド**（`<Primary>` = Ctrl）＋検証用の形の記録（`ctrl+KeyO` 等の解決済み綴り）で確認する。**ポップアップの画素そのものは見ていない**（7.5 がローカルで実測済み）。macOS は AX に権限が要るため**アプリが `set_menu` へ渡した配置と綴りの記録＋クレートのテスト**で検証する（**AppKit の描画そのものは証明しない**と明記）。Windows は Win32 のメニュー列挙＋`WM_COMMAND`。
...
- **10.6（堅牢化）**: mutter が最大化時に toplevel を作り直す事象への対策として**ウィンドウ集合が 2 回続けて同じになるまで待つ**、開始前に同題名のウィンドウが 0 件であることを要求、X エラーを説明可能な失敗に変換、キーバインドは `<Primary>` と `<Control>` の双方を許容（ランナーの GTK 版差の吸収）。
