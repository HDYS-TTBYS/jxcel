 ...
 *
 * ② **メニューからの貼り付けは 10.8 が結線した**（**複製のほうは 8.7 が結線した**）。8.7 の時点の
 * 障碍は**クリップボードを読む経路が無いこと**だった — 本アプリの読み口は DOM の `paste`
 * イベントだけであり（7.2 の設計）、メニューの活性化には `ClipboardEvent` が無い。
 * `tauri-plugin-clipboard-manager` は依存に無く（`src-tauri/Cargo.toml`）、
 * `navigator.clipboard.readText()` は 7.2 の実起動で**`不可`**（権限の拒否）と記録されている。
 * **読み口が無いまま項目を登録すると、`Ctrl+V` が基盤のメニューに取られて DOM の `paste` が
 * 届かなくなり、いま動いている打鍵の貼り付けを壊す** — 8.7 はその理由で登録しなかった
 * （`design.md` の「8.7 の時点の記録: 貼り付けの項目を登録しなかった理由」）。
 * **10.8 が読み口（公式プラグイン。利用者の決定 2026-09-17）を足して閉じた** — 器（`src-tauri`）が
 * クリップボードを読んで荷（`GridPasteRequestedEvent` の `text`）として送り、
 * `clipboardRequests.ts` が購読して**打鍵と同じ入口**（`RendererHandle.pasteText`）へ渡す。
 * **アクセラレータは付けない**（打鍵を 1 つも奪わない — 上の理由がそのまま契約として残る）。
 * 複製の側も**打鍵とメニューの双方が同じ入口**（`RendererHandle.copySelection`）へ着く（下の
 * [`createClipboardSurface`] と `clipboardRequests.ts`）。
 *
 ...
 * ③ **メニューからの実行**（要件 7.8）は **10.8 が結線した** — 器が「貼り付け」の項目を登録し
 * （`data-grid.paste`。**アクセラレータは付けない** — 付けると DOM の `paste` から打鍵を奪う）、
 * クリップボードを読んだ文字をイベントの荷として画面へ送る。画面は `./clipboardRequests` の
 * 購読で受け、**この module の `planPaste` へ流す**（打鍵とメニューが同じ入口へ着く）。
 * **この file の検査が観測しないのは ②（実機のクリップボードとの往復）だけである** — ③ の
 * 「荷の文字が既存の入口で適用されること」は `clipboardRequests.test.ts` が固定し、**器が実際に
 * クリップボードを読むこと**（本番の束縛）だけが実起動の観測に残る（9.2 の筋書き）。
 */
