 ...
 *
 * 500 ms は要件 11.2 の「最初の画面が 1 秒以内」の内側であり、12.2 が捕まえたい症状
 * （**無内容の領域のまま留まる**）に対して十分に短い。
 */
export const PAINT_PROBE_MS = 500;
...
export interface GridRenderHealth {
   ...
   * 不成立と読めてしまう（定数の doc）。**塗って読み戻せないことだけは取り直さない** —
   * 2D の文脈は待っても現れない。
   *
...
export function createGridRenderHealth(
  ...
  return {
    checkPaint: async (container: PaintProbeContainer | null): Promise<string | null> => {
      ...
      // **面とその中身は非同期に現れる**（[`PAINT_PROBE_MS`] の doc）。まだ現れていない
      // ことを表す 2 つの理由（面が無い・何も描かれていない）に限って、次のフレームで取り直す。
      // **塗って読み戻せないことは取り直さない** — 2D の文脈は待っても現れない。
      //
      ...
      for (
        ...
        now() < deadline &&
        !settled &&
        (outcome.failure === "no_canvas" || outcome.failure === "blank");
        attempt += 1
      ) {
        await nextFrame();
