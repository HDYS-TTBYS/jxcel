 ...
 *
 * **選択肢は境界が運ぶ**（10.3 が閉じた）: 宣言の選択肢は `ColumnDescriptor.choices` として届き、
 * `./columnConstraints` が `constraints.choices` へ写す（**この面は書き換えていない**）。
 * 渡されないとき（宣言に選択肢が無い・列がそもそも選択肢を持たない）は**値をそのまま扱う面**
 * （[`TextEditor`]）になる。
 *
 ...
 *
 * 既定へ落ちれば、値は**そのまま**編集できる（適合の判定は `schema-engine` が行う）。
 *
 * # 表の側（`select`）にしない理由
 *
