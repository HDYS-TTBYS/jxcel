 ...
 *
 * # 列数は作るときに 1 度だけ受け取り、変わったら作り直す
 *
 * 列数は**作るときに 1 度だけ**受け取る（[`createDisplayState`]）。渡るのは**描かれる列の数**で
 * あり、開いた直後は宣言の列数、入れ子を展開したあとは**導出後の構成の列数**である
 * （要件 5.1、5.2。`GridViewResponse.columns` を画面が採用し、`GridSurface` が器ごと組み直す）。
 * 列の集合と並び順はスキーマが供給し（requirements.md Boundary Context）、**列の追加・削除・型の
 * 変更はスキーマ編集を所有する機能の仕事である**（本機能の外）。したがって `columnOrder` は
 * **常に `0..列数-1` の置換**であり、長さが列数に足りない状態は作れない —
 * [`DisplayStateStore.moveColumn`] は置換を置換へ写すだけで、**配列の長さも要素の集合も変えない**。
 * 列数が変わったら状態を作り直す（`windowCache.clear` の事情「列の構成が変わったとき」と同じ扱い
 * であり、展開・折りたたみでは `GridSurface` の組み立ての効果がそれを担う）。
 *
 ...
 * いない — 開いた直後は宣言された列がスキーマの順に、展開・折りたたみのあとは**導出後の構成**の
 * 順にそのまま現れる（構成の並びが表示の位置の空間そのものである。要件 5.1、5.2）。`ViewSpec` の
 * 空の指定が「宣言の列がそのまま現れる」のと同じ規約である。
 *
