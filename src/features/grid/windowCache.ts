...
export interface WindowCache {
   ...
   * 来ている（境界は行の識別子しか運ばず、序数を求める手段はここにしか無い）。
   *
   * **いまは本番の経路から呼ばれていない**（10.5 が移動先の解決を応答の序数
   * `GridEditOutcome.affected_ordinals` へ移した。`./history` の module doc と design.md の
   * 10.5 の表）。残しているのは、能力そのものが本物であり、**7.3 の検査がこの口で序数と行の
   * 対応を固定している**ためである（`windowCache.test.ts` の 3 件 — `rowId` との往復・
   * 綴りの規律・捨てた窓の扱い）。**次に窓の記憶を触るときに、消すかどうかを決めること**
   * （design.md の 10.5 の申し送り）。
   *
...
export function createWindowCache(options: WindowCacheOptions): WindowCache {
  ...
  /**
   * その行の識別子が**いまの表示の順序で何番目か**（`rowId` の逆向きである）。
   *
