 ...
 * 受け取り、動いている DOM の `paste` が届かなくなるためである）。**10.8 が読み口を足して項目を
 * 登録した後も、この判断は変わっていない** — 項目は**アクセラレータを持たない**（`check-menu-shortcut.sh`
 * の期待が「ショートカット=(なし)」で固定し、`src-tauri` の検査が `Ctrl+V` を持つ項目が 1 つも
 * 無いことを固定する）。取り消しとやり直しは **逆向きの事情**にある — 打鍵を扱う経路が画面に
 * 1 つも無いので、アクセラレータは何も奪わない。この検査はその前提を**振る舞いで**固定する
 * （源の走査では、打鍵の表が別の名前に移ったときに追随できない）。
 */
