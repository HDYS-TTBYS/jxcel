 ...
 * - **メニューからの実行**（要件 7.8）は**両方とも `clipboardRequests.test.ts` が固定する**
 *   （イベント名が生成物から来ること、打鍵とメニューが同じ入口へ着くこと）。**貼り付けの項目は
 *   器が登録する**（`data-grid.paste`。10.8 がクリップボードの読み口を足して閉じた。**アクセラレータは
 *   持たない** — 付けると DOM の `paste` から打鍵を奪う。module doc「境界の項目」③）
 */
