 ...
 */
members: Array<ColumnMemberDescriptor>, };
/**
 ...
 * この並びは有限である**。
 */
export type ColumnMemberDescriptor = { 
/**
 * セル直下からの内側の位置（1 段以上）。
 */
path: Array<GridPathSegment>, 
 ...
 */
target: GridRowTarget, } | { "command": "DuplicateRows", 
/**
 * 複製する元の行（識別子の一覧、または可視の序数の区間）。
 */
target: GridRowTarget, } | { "command": "PasteRange", 
/**
 ...
 */
affected_ordinals: Array<number>, 
/**
 ...
 */
export type GridReferenceRequest = { 
/**
 ...
 * 文書に無い場合だけである。
 */
export type GridReferenceResponse = { 
/**
 * 呼び出し元ウィンドウの文脈。
 */
context: WindowContext, 
...
// 名指ししないため、境界が名指しできる具体形を明示的に置く。
export type GridReferenceResult = IpcResult<GridReferenceResponse, IpcError>;
/**
 ...
 */
export type GridRowAnchor = { "anchor": "Document", 
/**
 * 挿入する文書の位置（適用前の行順に対する添字。行数までの値が妥当）。
 */
at: number, } | { "anchor": "Before", 
/**
 * 対象の行の可視の序数。
 */
ordinal: number, } | { "anchor": "After", 
/**
 * 対象の行の可視の序数。
 */
ordinal: number, } | { "anchor": "End" };
/**
 ...
 */
export type GridRowTarget = { "target": "Ids", 
/**
 * 対象の行の識別子。空なら何も変えない。
 */
rows: Array<string>, } | { "target": "Ordinals", 
/**
 * 先頭の可視行の序数（含む）。
 */
from: number, 
