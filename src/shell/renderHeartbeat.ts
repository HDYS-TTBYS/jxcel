 ...
 *
 * `src/main.tsx` が**起動時の await をすべて終えた後・`createRoot().render()` の直前**に
 * 1 回だけ仕掛ける（`mountShell` の最後）。**仕掛けとマウントの間に await を挟んではならない**
 * — 通知は仕掛けた位置から数えた 2 フレームで出るので、間に非同期の処理（検証専用の動的
 * import など）があると、シェルがまだマウントされていないフレームから通知が出る。実測
 * （2026-09-12、macOS のランナー）: 動的 import の前に仕掛けると 2 フレームが import の完了より
 * 先に来て、領域が無い状態の通知（`画面=(報告なし)`）が確定し、10.4 の描画確認が落ちた。
 * **ウィンドウごとに別の JS 実行文脈なので、それぞれの文脈が自分の通知を送る**（送信先の
 * ウィンドウは Rust 側が注入された引数から取る。ここからウィンドウを申告しない。要件 4.6）。
 *
 ...
 *   受け入れ基準（通知を呼べば成立、呼ばなければ不成立）そのものが成立しない。**仕掛ける
 *   位置は起動時の await の後・マウントの直前である**（モジュール doc 冒頭の順序の制約）。
