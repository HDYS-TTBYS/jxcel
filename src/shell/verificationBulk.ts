 ...
 * TypeScript を括れないため、`src/main.tsx` は Vite の `define`（`vite.config.ts` の
 * `__JXCEL_VERIFICATION__`）で括られた分岐の中からだけ本モジュールを動的 import する。既定の
 * ビルド（`false`）ではその分岐が定数畳み込みで消え、**本モジュールの塊も生成されない**
