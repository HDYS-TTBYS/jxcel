...
#       「新規」「保存」— と `診断` の 3 件。配布物・検証用の形のそれぞれについて、`EXPECTED` の
#       各項目が木に現れることを要求する）。「新規」「保存」はドキュメントのセッション
...
#       **配置の記録は 9 項目すべてを要求する**（7.4 の「終了」・7.7 の「開く…」・9.5 の診断 3 件・
#       10.6 の検証専用 2 件に、**3.6 の「新規」「保存」**が加わった数である）。検証専用の
...
echo "検証 3/3: 配置の記録は期待どおり（位置・表示名・解決済みの綴り ctrl+… が 9 項目そろっている）"
