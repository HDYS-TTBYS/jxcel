...
#       「新規」「保存」— と `編集` の 1 件 — 8.7 の「複製」— と `診断` の 3 件。配布物・検証用の
...
#       **配置の記録は 10 項目すべてを要求する**（7.4 の「終了」・7.7 の「開く…」・9.5 の診断 3 件・
#       10.6 の検証専用 2 件・**8.7 の「複製」**に、**3.6 の「新規」「保存」**が加わった数である）。検証専用の
...
echo "検証 3/3: 配置の記録は期待どおり（位置・表示名・解決済みの綴り ctrl+… が 10 項目そろっている）"
