...
#       「新規」「保存」— と `編集` の 4 件 — 8.7 の「複製」・**8.9 の「元に戻す」「やり直し」**・
#       **10.8 の「貼り付け」（キーバインド無し）** — と、`macro-runtime` の 4.3 が足した
#       `マクロ` の 1 件（**「実行…」。キーバインド無し**）と `診断` の 3 件。配布物・検証用の
...
#       **配置の記録は 14 項目すべてを要求する**（7.4 の「終了」・7.7 の「開く…」・9.5 の診断 3 件・
#       10.6 の検証専用 2 件・**8.7 の「複製」**・**8.9 の「元に戻す」「やり直し」**・
#       **10.8 の「貼り付け」**・**4.3 のマクロの「実行…」**に、
...
EXPECTED = {
    "shipping": {
        ...
        # 10.8 が足した「編集 > 貼り付け」（要件 7.8 の後半）。**キーバインドを持たない** —
        # 器が読んだクリップボードの文字を送るだけで、打鍵（DOM の `paste`）の経路を
        # 1 つも奪わない（`Ctrl+V` を載せれば、基盤のメニューが打鍵を先に受け取る環境で
        # DOM の `paste` が届かなくなる）。
        "編集": {
            "複製": (("primary",), "c"),
            "元に戻す": (("primary",), "z"),
        ...
        # `マクロ` は `macro-runtime` スペックのタスク 4.3 が足した（要件 2.1）。**配布物にも
        # 現れる**（`data-grid.copy` と同じ扱いである）。**キーバインドを持たない** —
        # 要件はキーボードの導線を求めておらず、付ければ打鍵を 1 つ奪うだけである。
        "マクロ": {
            "実行…": ((), None),
        },
        "診断": {
...
for expected in \
  ...
  "data-grid.paste(編集 > 貼り付け, ショートカット=(なし))" \
  ...
  "macro-runtime.run(マクロ > 実行…, ショートカット=(なし))"
do
  case "$placement" in
    *"$expected"*) ;;
    *)
...
echo "検証 3/3: 配置の記録は期待どおり（位置・表示名・解決済みの綴り ctrl+… が 14 項目そろっている）"
