...
#      対して検査する。
#   2. **初回描画が成立したこと** — 記録に 8.2 の成立行（`初回描画が成立した: label=…`、
#      またはソフトウェアラスタライザ経由の `初回描画は成立したがソフトウェアラスタライザ
#      経由である: label=…`）が現れることを要求する。**これが「最初のフレームが描かれた」
...
#      `初回描画が成立しなかった: …` を記録するため、失敗の理由は記録の末尾に出る。
...
# 記録に求める行。**成立行は 2 つある**（8.2 の判定は三値であり、`Painted` と
# `SoftwareRaster` のどちらも描画の成立である）。
...
while [ "$(date +%s)" -lt "$render_deadline" ]; do
  if [ -f "$record" ]; then
    ...
    # 8.2 が期限超過を確定したら、猶予の残りを待たずに落とす（失敗の理由が記録そのものにある）。
    if [ -z "$heartbeat_line" ]; then
      no_paint=$(grep -F '初回描画が成立しなかった' "$record" | tail -n 1 || true)
      if [ -n "$no_paint" ]; then
        report_failure "8.2 が初回描画の不成立を記録した: ${no_paint}"
      fi
    fi
  fi
