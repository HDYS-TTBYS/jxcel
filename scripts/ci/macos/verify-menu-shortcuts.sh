...
# であり、**8.7 の「複製」と 8.9 の「元に戻す」「やり直し」は `編集` に**入り、診断の 3 項目は 診断 に入る。**「新規」「保存」はアプリケーションメニューの項目ではない** —
...
echo "OK: macOS: アプリ全体のメニュー（配置=アプリ全体）に 12 項目が期待の位置・表示名で並び、ショートカットは super+…（Cmd）へ解決されている"
