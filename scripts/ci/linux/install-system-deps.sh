...
# **`openbox` を明示する理由**: 9.2 の観測の段は仮想ディスプレイに**最小のウィンドウ
# マネージャ**を載せる。ウィンドウマネージャが無いと GTK の「活性のある窓」が決まらず、
# メニューの活性化が対象ウィンドウへ届かない（CI の実測: 貼り付けの活性化が成立として
# 返るのに、器は貼り付けの要求を記録しなかった）。**実機のデスクトップと同じ前提**にする。
