# Windows の段（**tasks.md 1.6**）。走査の使い捨ての画面（`smoke-glide-probe`）が Windows で
# **実際に描画される**ことを起動して確かめる。観測は 10.4 の段と同じ `MainWindowHandle` と
# 診断記録を使う（**この段のために新しい系統を作らない** — 1.6「既存の 3 OS の検証マトリクスに
# 一時的な段を足し、新しい系統を作らない」）。
#
# # 数値をここでは出さない（1.6 の明文）
#
...
# **アクセシビリティの木**へ出す 1 行から読むが、**Windows には AT-SPI が無い**（AT-SPI は
# D-Bus 上の仕組みであり、WebView2 は UI Automation を使う。Linux の検査器
# `scripts/check-render-traversal.sh` を Git Bash で呼んでも、計測の行を読む経路が無い）。
# したがって**この段は Windows で数値を出さない**。ここで確かめるのは次の 2 つである:
#
#   1. 走査の画面が WebView2 で**実際に描画される**こと（`画面=smoke-glide-probe` の成立行）。
#   2. 10 万行の走査の画面が Windows で**起動できる**こと（ウィンドウが現れ、初回描画が成立する）。
#
# **数値の欠落を「成立」と読み替えない** — この段が緑でも、要件 11.1 の Windows の証拠には
# ならない。恒久の観測は 9.2 が担う。
#
# # 一時的な段である
#
...
Write-Host "注記: この段は走査の画面が Windows で描画されることだけを確かめました。"
Write-Host "注記: 走査中のフレーム時間の中央値の Windows の数値は、1.6 の明文により 9.2 の観測で確かめます。"
