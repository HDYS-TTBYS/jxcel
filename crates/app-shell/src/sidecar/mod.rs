...
//! 3.3 がプラットフォーム別の終了保証、3.4 が行単位の出力取得と予期せぬ終了の通知、
//! 3.5 が残留の掃除（[`orphan_sweep`]）と親監視の引数の注入を追加した。
...
impl SidecarKind {
    ...
    /// 実行ファイルの語幹から種類へ戻す。未知の名前は `None` を返す。
    ///
    /// **厳密一致に限る。** 拡張子やターゲットトリプル接尾辞を含む名前は語幹ではないため解釈
    /// しない — 照合が緩むと無関係なプロセスを誤って対象にしうる。残留プロセスの掃除（要件 5.6、
    /// タスク 3.5）は配置名（`sidecar-smoke` と `sidecar-smoke-<ターゲットトリプル>[.exe]`）の
    /// 双方を扱う必要があるため、この関数ではなく `orphan_sweep::identify` が語幹の一致までを
    /// 見て種類を推測し、8.1 が与える期待パスとの照合で曖昧さを締める。
    pub fn parse(name: &str) -> Option<SidecarKind> {
        Self::ALL.iter().copied().find(|kind| kind.as_str() == name)
    }

    /// 親の識別子を監視するためのコマンドラインフラグ。監督が起動時に**自分の識別子を渡す**
    /// ために使う（要件 5.6、tasks.md 3.5）。
    ///
    /// **種類ごとに決める理由**: 補助プロセスのコマンドラインは種類ごとに異なり、将来の種類が
    /// このフラグを受け付けるとは限らない。`Some(フラグ)` を返す種類は「自身のコマンドラインに
    /// そのフラグがあり、値として監視対象の識別子を受け取る」ことを表明する（監督はこれを
    /// 注入する）。`None` を返す種類には監督は何も注入しない。
    ///
    /// **新しい種類はここで明示的に選ぶ。** 変種を足せばこの `match` がコンパイルエラーに
    /// なるため、受け付けるかどうかを決めずに素通りすることはできない。
    pub const fn parent_pid_flag(self) -> Option<&'static str> {
        match self {
            SidecarKind::Smoke => Some("--parent-pid"),
        }
    }
}

/// `as_str` と同じ表現を `Display` でも返す。終了失敗（[`supervisor::ShutdownError`]）の報告に、
/// 同梱する実行ファイル名と同じ識別子を載せるためである。
