...
//!
//! **タスク 10.3 が 7 本目を足した**（[`GridReferenceRequest`] / [`GridReferenceResponse`]。
//! 要件 3.8）。参照先の行を**頁ごとに**読む経路であり、要求が件数を運び、上限
//! （[`GRID_REFERENCE_PAGE_LIMIT`]）を**境界が強制する** — 参照先が 1 万行でも一度に全部を
//! 読まない。あわせて [`ColumnDescriptor`] が**宣言から導ける材料**（選択肢・参照先のシートの
//! 名・ユーザー定義型の識別子・値なしを許すか・入れ子の内側の宣言）を運ぶ。
//!
...
/// 選択肢を持つ型（`Enum`）の 1 件（タスク 10.3。要件 3.2）。
///
/// **値と名を別々に持つ。** 宣言（`schema-engine` の `ColumnValidator::Enum`）が持つのは
/// 1 つの文字列の並びだけであるため、いまは両者が同じ文字列になる — それでも欄を 2 つに
/// しておくのは、**面が値と名を別々に描ける**ようにするためである（確定するのは値であり、
/// 並べるのは名である。1 つに潰すと、値と表示が同じであることが面の側の前提になる）。
/// 宣言が 2 つを分けて持つようになった日に、変わるのは写す側（適応層）1 箇所で済む。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ts_rs::TS)]
pub struct ColumnChoice {
    /// 確定する値（宣言に書かれた文字列そのもの）。
    pub value: String,
    /// 一覧に並べる名。
    pub label: String,
}

/// 入れ子の内側の位置 1 つの**宣言**（タスク 10.3。要件 5.5）。
///
/// **展開していない列でも読める**ことが要点である — 構成の要素（[`ColumnDescriptor`]）は
/// 展開の状態に依るため、折りたたまれた入れ子の内側は構成に現れない。詳細表示（要件 5.5）と
/// 入れ子の入力の面（要件 5.5、5.7）は、それでも内側に何が宣言されているかを要る。
///
/// 位置（[`ColumnMemberDescriptor::path`]）は**セル直下からの絶対の位置**であり、
/// [`ColumnDescriptor::path`] と同じ空間に属する（違反の位置（要件 4.5）と直接突き合わせて
/// よい）。深さは列として展開する上限（要件 5.4）で切られており、**どれだけ深い宣言でも
/// この並びは有限である**。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ts_rs::TS)]
pub struct ColumnMemberDescriptor {
    /// セル直下からの内側の位置（1 段以上）。
    ...
    /// その位置が値なしを許すか（そのフィールドの `required` から写す。要件 3.7）。
    pub nullable: bool,
    /// 選択肢（`Enum` の位置のみ。空なら材料なし。要件 3.2）。
    pub choices: Vec<ColumnChoice>,
    /// ユーザー定義型の識別子（`Custom` の位置のみ。要件 10.1、10.4）。
    pub custom_type_id: Option<String>,
}
...
/// 展開の可否、そして**宣言から導ける材料**（タスク 10.3）である。`design.md`
/// 「GridSession」の Implementation Notes が「6.1 はここから境界型へ写す」と定めているのは
/// この型である。
///
...
pub struct ColumnDescriptor {
    ...
    /// 値なしを許すか（列の `required` の否定。要件 3.7。タスク 10.3 が足した）。
    ///
    /// **宣言から写す。**定数にしない — 値なしを許さない列で「値なしへ戻す」道を出すと、
    /// 判定が違反を返す値を作れてしまう（要件 3.5）。
    pub nullable: bool,
    /// 選択肢（`Enum` のみ。宣言順。空なら材料なし。要件 3.2）。
    pub choices: Vec<ColumnChoice>,
    /// 参照先のシートの**名**（`Ref` のみ。`None` なら参照しない列。要件 3.8）。
    ///
    /// 宣言が持つのはシートの**識別子**であり（`schema-engine` の `ColumnValidator::Ref`）、
    /// 人が読む名は文書が持つ。**名への写しは適応層が行う**（文書を見られるのはあちらだけ
    /// である）。参照先のシートが文書に無い場合は、識別子をそのまま載せる（名が引けないこと
    /// を「参照していない」と混同させない — 面は識別子を出し、行の一覧は空になる）。
    pub reference_sheet: Option<String>,
    /// ユーザー定義型の識別子（`Custom` のみ。登録簿を引く鍵。要件 10.1、10.4）。
    pub custom_type_id: Option<String>,
    /// 入れ子の内側の宣言（**展開していない列でも読める**。要件 5.5）。
    ///
    /// 空の並びは「内側を持たない（または宣言が深さの上限で切られた先である）」を意味する。
    /// 面はそのとき位置ごとの面を出さず、値をそのまま扱う既定へ落ちる（要件 10.4）。
    pub members: Vec<ColumnMemberDescriptor>,
}
...
///
/// `edit` 層の `RowTarget` を写したもので、**2 つの腕は同じ集合を別の座標空間で名指す**。
///
/// | 腕 | 座標空間 | 誰が解くか |
/// |---|---|---|
/// | `Ids` | **文書の同一性**（行の識別子） | 解くものが無い |
/// | `Ordinals` | **可視の序数**（半開区間） | ドメインが適用の直前に `RowOrder` で解く |
///
/// **画面は序数を文書の位置へ写さない**（写像を持つのはドメインだけである。要件 8.6）。
/// したがって `Ordinals` の腕には**可視行の序数だけ**を載せる — 文書の位置を載せると、
/// 並べ替えや絞り込みが効いている間に別の行を消す。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ts_rs::TS)]
#[serde(tag = "target")]
pub enum GridRowTarget {
    /// 行の識別子の一覧（文字列表現。64 ビット整数を境界へ出さない）。
    Ids {
        /// 対象の行の識別子。空なら何も変えない。
        rows: Vec<String>,
    },
    /// **可視の序数の半開区間** `from ..< from + count`。
    Ordinals {
        /// 先頭の可視行の序数（含む）。
        from: u32,
        /// 区間の行数。
...
///
/// `edit` 層の `RowAnchor` を写したもので、**文書の位置**と**可視の序数**の双方を表せる。
/// 画面が持っているのは表示の位置であり、文書の位置へ写す口は境界に無い — 写すのは
/// **ドメイン（適用の直前）** である（要件 8.6。写せないことを理由に操作を拒まない）。
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, ts_rs::TS)]
#[serde(tag = "anchor")]
pub enum GridRowAnchor {
    /// 文書の行順に対する位置（`at == 行数` は末尾への追加）。
    Document {
        /// 挿入する文書の位置（適用前の行順に対する添字。行数までの値が妥当）。
        at: u32,
    },
    /// **可視の序数 `ordinal` の行**の直前。
    Before {
        /// 対象の行の可視の序数。
        ordinal: u32,
    },
    /// **可視の序数 `ordinal` の行**の直後。
    After {
        /// 対象の行の可視の序数。
        ordinal: u32,
    },
...
///
/// 行の対象（`RemoveRows::target` / `DuplicateRows::target`）と挿入の位置
/// （`InsertRows::at`）は、いずれも**2 つの座標空間**を表せる（[`GridRowTarget`] /
/// [`GridRowAnchor`]。タスク 10.4）— 画面は表示の序数だけを送り、文書の位置へ写すのは
/// ドメインである（要件 8.6）。貼り付けは起点と**表示されている行の並び**の 2 つで宛先が
/// 決まる（要件 8.9）ため、`PasteRange::rows` を持つ。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ts_rs::TS)]
#[serde(tag = "command")]
pub enum GridEditCommand {
    ...
    /// 指定した**位置**へ、宣言の既定値を持つ行を `count` 行足す（要件 6.1）。
    InsertRows {
        /// 挿入する位置（文書の位置、可視の序数の前後、末尾のいずれか）。
        at: GridRowAnchor,
        /// 挿入する行数。`0` は何も変えない。
        count: u32,
    },
    /// 選択された行を 1 回の操作として取り除く（要件 6.2）。
    RemoveRows {
        /// 取り除く行（識別子の一覧、または可視の序数の区間）。
        target: GridRowTarget,
    },
    /// 選択された行と同じ値を持つ行を末尾へ足す（要件 6.3、6.4）。
    DuplicateRows {
        /// 複製する元の行（識別子の一覧、または可視の序数の区間）。
        target: GridRowTarget,
    },
...
///   （要件 1.7）ために使う。
/// - `affected_ordinals` — 影響を受けた行のうち、**整えたあとの表示の並びに在るもの**の
///   **表示の序数**（可視行の添字。重複を畳み、`affected` と同じ順）。画面はこれで現在位置を
///   移す（要件 9.8）— **窓の記憶がその行を保っているかどうかに依らない**。写せるのは
///   `RowOrder` を持つ適応層だけであり、**写せない行（順序に無い行）は含めない**
///   （推測した序数は無関係な行を名乗る）。
...
pub struct GridEditOutcome {
    ...
    /// 影響を受けた行の**表示の序数**（可視行の添字。重複を畳み、`affected` と同じ順）。
    ///
    /// **`affected` の部分集合である** — 写せなかった行（順序に無い行。削除で消えた行、
    /// 絞り込みで隠れた行、別のシートの行）は入らない。両方の欄が同じ「影響を受けた行」の
    /// 集合を表し、こちらは**表示の並びで位置が言えるものだけ**である（番号が対応するわけでは
    /// ない — 写せない行を飛ばすため、添字は一致しない）。
    ///
    /// 値を作るのは**適応層**（`src-tauri/src/commands/grid.rs` の `outcome_to_boundary`）で
    /// あり、**適用の後（順序を導出し直した後）の `RowOrder`** から写す（要件 9.8）。画面は
    /// 写像を持たない（要件 8.6 の取り違えを写しの側から作らない）。
    pub affected_ordinals: Vec<u32>,
...
/// 次の違反を探す要求（タスク 6.2。要件 4.4）。**セルを指定する要求でもある**（タスク 10.6。
/// 要件 4.2）。
///
...
pub struct GridViolationRequest {
    /// 探索の起点（**可視行の 0 起点の序数**）。[`Self::column`] を指定したときは、
    /// **そのセルの行**を指す。
    pub from: u32,
    /// 探索の向き。
    pub direction: GridSearchDirection,
    /// **指定したセルの文書の列**（`None` なら指定が無い。タスク 10.6。要件 4.2、4.5）。
    ///
    /// - `Some(column)` — `from` の序数の行の**その列のセル**の違反を答える。**探索はしない**
    ///   （宛先が 1 つに定まるため `direction` は使わない）。そのセルが違反していなければ
    ///   `violation: None` である（**同じ行の別の列の理由を名乗らない** — 行の最小の違反列を
    ///   返す従来の答えでは、利用者が指したのが右のセルでも左のセルの理由になっていた）。
    /// - `None` — 従来どおりである（`from` と `direction` から最も近い違反セルを探し、その行の
    ///   最小の違反列を答える）。
    ///
    /// 値は**文書の列**（`Row::values()` に対する位置）である。画面が表示の位置から写して送る
    /// （写像は `ColumnSpace` の 1 箇所であり、画面は持たない。要件 8.6）。**写せないときは
    /// `None` を送る** — 推測した列を送れば、画面が名乗っていないセルの理由をそのセルの理由と
    /// して見せることになる。
...
// ---------------------------------------------------------------------------
// 参照先の行（タスク 10.3。要件 3.8）
//
// 7.4 の申し送り 2（「参照先のシートと、その行を一覧する経路が 6 本のコマンドに無い」）を
// 閉じる **7 本目のコマンド**である。**参照先は別のシートであり、その行数は表示中のシートと
// 無関係である**（1 万行の参照先は普通にありうる）ため、応答は**頁**に閉じる — 要求が件数を
// 運び、[`GRID_REFERENCE_PAGE_LIMIT`] が上限を強制する。
//
// 名前は「参照先の行」そのものである（`grid_reference_rows`）。**要求は文書の列の添字だけを
// 運ぶ** — 参照先のシートは宣言から決まる（`ColumnDescriptor::reference_sheet`）ので、画面が
// シートの識別子を持ち回る理由が無い（写しを持つと、宣言と食い違いうる）。
// ---------------------------------------------------------------------------

/// 参照先の行を 1 頁読むときに、境界が許す**件数の上限**（タスク 10.3。要件 3.8）。
///
/// **要求がこれを超えたら、境界が上限へ切り詰める。**上限が要るのは、参照先が 10 万行でも
/// **一度に全部を読まない**ためである（要件 11 の目的。応答の大きさが要求から決まると、
/// 画面は「全部読む」要求を 1 つ書くだけで境界を溢れさせられる）。
///
/// 切り詰めは**要求の値ではなく応答の件数に効く**（`has_more` が真になるので、画面は続きを
/// 読める）。切り詰めたことを失敗として返さないのは、要求が大きすぎることは利用者の誤りでも
/// 経路の失敗でもないためである（§ 6.2 の写像の規律と同じ扱いである）。
pub const GRID_REFERENCE_PAGE_LIMIT: u32 = 200;

/// 参照先の行を頁ごとに読む要求（タスク 10.3。要件 3.8）。
///
/// ウィンドウは要求の型に現れない（呼び出し元は基盤が注入する。要件 4.6）。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ts_rs::TS)]
pub struct GridReferenceRequest {
    /// **文書の列の添字**（可視の位置でも表示の位置でもない。要件 8.6）。
    ///
    /// 参照先のシートは、この列の宣言（`ref` の `sheet`）から決まる。参照しない列を指定した
    /// 場合は**経路の失敗**である（「行が無い」と混同しない — 6 本の写像と同じ規律）。
    pub column: u32,
    /// 検索の文字。**空なら絞り込まない。**
    ///
    /// 一致するのは行の**表示の名**（その行の値の表示文字列を列の順に空白で連結したもの）
    /// に対する部分一致である（規則は `data-grid` の `view::reference` が唯一の源）。
    pub search: String,
    /// 読み始める位置（0 起点。絞り込んだあとの並びに対する位置である）。
    pub start: u32,
    /// 読む件数。**[`GRID_REFERENCE_PAGE_LIMIT`] を超える値は上限へ切り詰められる。**
    pub count: u32,
}

/// 参照先の 1 行（タスク 10.3。要件 3.8）。
///
/// **確定するのは `id`（行の識別子）であり、人が読むのは `label` である。**参照の値は
/// 「どの行か」を指すのであって、その行の表示文字列ではない（表示は並べ替えや絞り込みで
/// 変わる。要件 8.5、8.6）。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ts_rs::TS)]
pub struct GridReferenceRow {
    /// 行の識別子（参照の列に書かれる値そのものである）。
    pub id: String,
    /// 人が行を見分けるための表示の名。
    pub label: String,
}

/// 参照先の行の 1 頁（タスク 10.3。要件 3.8、4.6）。
///
/// **呼び出し元ウィンドウの文脈を必ず含む**（要件 4.6）。`rows` は `start` から数えた頁であり、
/// **`GRID_REFERENCE_PAGE_LIMIT` を超えない**。`total` は**絞り込んだあとの総数**であり
/// （頁の大きさではない）、`has_more` は「この頁の後ろにまだ行があるか」である。
///
/// **行が 1 つも無いことは正常な結果である**（参照先のシートが空であるか、検索の文字に何も
/// 一致しない）。封筒の失敗腕へ落ちるのは、その列が参照の型でない場合と、参照先のシートが
/// 文書に無い場合だけである。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ts_rs::TS)]
pub struct GridReferenceResponse {
    /// 呼び出し元ウィンドウの文脈。
    pub context: WindowContext,
    /// この頁の行（文書の行順）。
    pub rows: Vec<GridReferenceRow>,
