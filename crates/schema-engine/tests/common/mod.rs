...
//! `schema-engine` のスキーマ宣言（`declaration`）は参照しない。ここが提供するのは
//! 「指定した行数・列数のシートと行の値」と「違反として仕込む値の割合」だけである。
//!
//! 宣言を必要とする標本（型つきの 30 列）は、この境界の外にある子モジュール
//! [`schema`] が生成器の上に載せる（tasks.md 9.1。要件 10.6）。子モジュールは
//! `schema-engine` の公開面と上流の値に依存し、本モジュールの非公開の道具
//! （[`encode_rows`] と [`with_rebuilt_manifest`]）を再利用する — 2 つのシートを
//! 1 回の復元で組む標本のために、行エントリの符号化を写し取らないためである。
//! ベンチは `#[path]` で本モジュールを取り込むため、子モジュールも同じ経路で届く。
//!
//! # 10 万行を O(n) で組み立てる理由
//!
