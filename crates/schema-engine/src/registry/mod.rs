...
pub trait CustomType: Send + Sync {
    ...
    /// 判定を、`values` と同じ順で渡す。
    ///
    /// # `Err` を返すときに上書き実装が守ること（tasks.md 4.1 の裁定）
    ///
    /// **`Err` を返す前に、失敗した値より前の判定をすべて `out` へ出現順に渡してから
    /// 返す。** まとめて判定してから流す実装（境界を越える実装に多い）は、途中で失敗した
    /// ときに**それまでに集めた分を先に流してから** `Err` を返す。`validate` 層は「`out` が
    /// 受け取った件数 `k`」を**失敗した値の位置**とみなし、その 1 件を違反として報告して
    /// `k + 1` 番目から一括判定をやり直す（要件 11.5 の隔離と走査の継続。`src/validate` の
    /// docs「拡張型の列は列ごとに 1 回の一括判定で判定する」）。この契約を破ると、失敗より
    /// 前の値の判定が失われるか、同じ値が二度報告される。既定実装は `out(i, self.validate(v)?)`
    /// の順で書かれているため、この契約を自然に満たす。
    fn validate_batch(
        &self,
        values: &[CellValue],
