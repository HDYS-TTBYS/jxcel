...
//! # 現状（tasks.md 群 4）
//!
//! タスク 4.2 が [`plan`]（列 1 本分の検証器。`ColumnValidator`）を置いた。タスク 4.3 が
//! [`resolve`]（`$ref` の解決と、値が有限の大きさで存在しえない循環の検出。要件 3.5, 3.6）
//! を置いた。`SchemaCompiler` 本体（`CompiledSchema` と列の並び順の供給。要件 1.1, 1.2,
//! 1.8, 11.7）はタスク 4.4 が本モジュールに足す。タスク 4.4 は本層の [`resolve`] を使って
//! 参照を解決し、[`plan`] の検証器を組み立てる。
