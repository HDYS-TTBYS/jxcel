...
//! 待つ操作と待たない操作を分ける:
//!
//! - **待つ**: [`Slot::read`] / [`Slot::state`] / [`Slot::discard`] は文書のロックを待って
//!   取る。適用中に到着した読み取りには適用済みの最新を返す（要件 3.4。テストだけが使う
//!   `with_document_mut` も同じ）
...
//! # 再入禁止
//!
//! [`Slot::read`]（と、テストだけが使う `with_document_mut`）の閉包の内側から**同じセッションを
//! 呼び返してはならない**。`Mutex` は再入できないため、文書のロックを取る操作（[`Slot::read`] /
//! [`Slot::state`] / [`Slot::discard`] と、タスク 2.2 以降の保存）を呼ぶとデッドロックする。
//! 例外は [`Slot::may_close`] であり、文書のロックを取らないため閉包の内側からでも安全である。
//!
//! # テストだけが使う記録しない借出口
//!
//! `Slot::with_document_mut` は文書を可変で貸すが、**未保存の印と版を変えない**。したがって
//! **production の可変の経路は `change::edit` ただ 1 つでなければならない**（借出口を
//! 迂回すると、文書が変わるのに記録が残らず、要件 4.1 の抜け道になる）。production からの
//! 呼び出しが無いことを確かめたうえで、本関数は `#[cfg(test)]` に閉じている（タスク 2.5 の
//! 判断。`with_document_mut` の doc を参照）。
//!
//! 適用の記録（版を 1 進め、未保存の印を立てる）は変更の適用口（`change::edit`）の責務である。
//!
...
/// モジュール docs「ロックの規律」）。
///
/// 本型は crate 可視であり、根の再輸出には現れない（下流が使うのは公開面の
/// [`DocumentSessions`](crate::DocumentSessions) の名前だけである）。
...
impl Slot {
    ...
    /// 保持している文書を可変で借りる（**記録しない**借用の口である）。**テストだけが使う。**
    ///
    /// **未保存の印と版はここでは変えない**（モジュール docs「テストだけが使う記録しない
    /// 借出口」）。記録まで含む変更の適用は `change::edit` が [`Slot::lock_for_change`] /
    /// [`Slot::record_edit`] で行う。未解決なら [`SessionError::NoDocument`]。閉包の内側から
    /// セッションを呼び返してはならない（モジュール docs「再入禁止」）。
    ///
    /// **`#[cfg(test)]` に閉じる理由**（タスク 2.5 の判断）: production の可変の経路は
    /// [`crate::change::edit`] ただ 1 つでなければならない。本関数を production から使うと、
    /// 文書は変わるのに未保存の印と版が立たない**抜け道**になる（要件 4.1「変更が適用された
    /// とき、そのウィンドウを未保存として扱う」が破れる）。本クレート全体を検索して production
    /// の呼び出しが無いことを確かめたうえで、テスト（「ロックを保持したまま、記録せずに
    /// 書き換える」操作の観測）のためにだけ残す。
    #[cfg(test)]
    ...
    ///
    /// この入口が要るのは、テストだけが使う `with_document_mut` が**記録しない**契約だから
    /// である（モジュール docs「テストだけが使う記録しない借出口」）。記録まで含む口を
    /// `session` に置くと「変更の適用」の意味論（変更の語彙を持たないこと・閉包の失敗でも
    /// 記録する保守側への倒し方）が状態機械へ混ざるため、状態機械は**貸す口**だけを与え、
    /// 意味論は `change` が持つ。
