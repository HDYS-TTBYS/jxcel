...
//!
//! 名前の列は ZIP のバイト列表現（UTF-8。6 形はすべて ASCII）として得る。UTF-8 として
//! 読めない生の名前は 6 形のいずれにも一致し得ないため、この段階で拒否する
//! （唯一 `EntryName::parse` を経由しない拒否であり、理由は「`parse` が `&str` を要求する」
//! ことに尽きる。`&str` でない名前はこの 6 形の外である）。
//!
...
///
/// 実在するエントリ名が無い失敗（ZIP として開けない等）に使う。許可リストの 6 形の
...
mod tests {
    ...
    /// UTF-8 として読めない生のエントリ名を拒否する（6 形はすべて ASCII）。
    #[test]
