...
//! | 圧縮方式 | 型マーカーが `Stored`、他が `Deflate`（レベル 2。`FIXED_COMPRESSION_LEVEL`） | crate の既定（deflate バックエンド・`flate2` の既定レベル）に依存する |
//!
//! ## 型マーカー
//!
...
///
/// 値は保存の予算（10 万行 × 30 列を 2 秒以内。要件 8.2）から決めた。当初の 6
/// （`flate2::Compression::default()` と同じ値）では圧縮が保存時間の 6 割を占め、CI の
/// 全ランナーで予算を超えた。`miniz_oxide` で 10 万行 × 30 列（非圧縮 44.6 MB）を
/// 実測した値は次のとおりで、2 がサイズと時間の折り合う点である（3 は 2% 小さいだけで
/// 2.3 倍遅く、1 は 1 割速いだけで 3 割大きい）:
///
/// | レベル | 圧縮後 | 時間 |
/// |--------|--------|------|
/// | 1 | 12.1 MB | 86 ms |
...
/// | 6 | 7.2 MB | 883 ms |
///
/// レベルは読み手に影響しない（Deflate の伸長はレベルを問わない）ため、形式バージョン
/// は変えない。
const FIXED_COMPRESSION_LEVEL: i64 = 2;
