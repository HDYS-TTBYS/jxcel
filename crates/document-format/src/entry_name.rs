...
//!
//! 受理する 6 形の集合は形式バージョン [`crate::error::FormatVersion`] と同じ場所で、
//! 同じ審査で管理する（`FormatVersion` は暫く `error.rs` が所有しタスク 6.1 で
//! `migration` へ移管される。移管時は本注記を移管先へ追従させる）。
//! [`LAYOUT_FORMS`] がその単一審査可能集合であり、集合の変更は形式バージョンの
//! 変更として扱う。
//!
//! # 順序
//!
