...
//!
//! スキーマ・エンベロープ（`src/model/schema_part.rs`）の
//! [`RawField`](crate::model::RawField) / `SchemaPart::unknown_fields` は、
//! **エンベロープ・トップレベルに限定した**同じ考え方の特殊化である（タスク 2.2 が
//! 先に確定させた）。`schemas/<sheet-ulid>.json` の最終的なバイト再構成は
//! タスク 4.4（SchemaCodec）が本ファイルのプリミティブを使って担う想定であり、
//! 2 つの機構が併存するのはこの分担のためである（エンベロープ専用の保持を
//! モデル層に置き、再利用可能な一般形を json 層に置く。一般形を model 層の型へ
//! 寄せないのは依存方向 `Model → Json` を崩さないため）。
//!
//! # 失敗時の保証
//!
