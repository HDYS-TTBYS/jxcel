...
//!
//! これは配布しない検証専用の相手役である（design.md「Out of Boundary」、research.md 決定 9:
//! 実物の言語サーバを登録するのは下流の `macro-editor-lsp` である）。**実用的な機能を足しては
//! ならない。** プロトコル・JSON・IPC・引数解析フレームワーク・記録機構・設定読み取りは
//! 意図的に持たない。ここに機能が増えた時点で、検証したい機構（tasks.md 3.x）と検証対象が
//! 混ざり、何を確かめているのか分からなくなる。
//!
//! # 本プロセスを消費するタスク
//!
...
//!
//! ```text
//! sidecar-smoke --parent-pid <PID>
//! ```
//!
//! `<PID>` は監視する親プロセスの識別子で、1 以上の整数に限る。`0` は Unix では
//! プロセスグループを指してしまい「消えた」ことを検出できないため受け付けない。
//! 引数の欠落・未知の引数・数値でない値・範囲外の値は、標準エラー出力へ使い方を出して
//! 終了コード 2 で即座に終了する（ハングしない）。
//!
//! # 標準出力の形式
//!
...
fn main() {
    ...
    // 起動行。監視対象と自身の識別子を 1 行で示し、行単位の購読側に起動を伝える。
    {
        ...
        let _ = writeln!(
            out,
            "sidecar-smoke ready pid={} parent_pid={}",
            std::process::id(),
            parent_pid
        );
        let _ = out.flush();
...
/// `--parent-pid <PID>` だけを受け付ける。余分な引数や不正値は `None`。
fn parse_parent_pid<I>(mut args: I) -> Option<u32>
where
    I: Iterator<Item = String>,
{
    ...
    let raw = args.next()?;
    // 余分な引数は受け付けない（黙って無視しない）。
    if args.next().is_some() {
        return None;
    }
    let pid: u32 = raw.parse().ok()?;
    // 0 は Unix でプロセスグループを指し、存在確認が常に成功してしまう。
    // 符号付き 32 ビットを超える値は pid として不正である。
...
fn print_usage() {
    eprintln!("使い方: sidecar-smoke --parent-pid <PID>");
